import { Icon, IconProps } from "@chakra-ui/react"
import * as React from "react"

export const Location = (props: IconProps) => (
  <Icon viewBox="0 0 24 24" fill="none" {...props}>
    <path
      d="M12 13.43C13.7231 13.43 15.12 12.0331 15.12 10.31C15.12 8.58687 13.7231 7.19 12 7.19C10.2769 7.19 8.88 8.58687 8.88 10.31C8.88 12.0331 10.2769 13.43 12 13.43Z"
      stroke="currentColor"
      strokeWidth={1.5}
    />
    <path
      d="M3.62001 8.49C5.59001 -0.170002 18.42 -0.160001 20.38 8.5C21.53 13.58 18.37 17.88 15.6 20.54C13.59 22.48 10.41 22.48 8.39001 20.54C5.63001 17.88 2.47001 13.57 3.62001 8.49Z"
      stroke="currentColor"
      strokeWidth={1.5}
    />
  </Icon>
)
